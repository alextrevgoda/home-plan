...
- **Two fingers:** pinch-zoom about the finger midpoint with simultaneous two-finger pan, built on the existing `zoomAt` (zoom limits already clamped 5–400 px/m). If a second finger lands mid-drag of an item, the drag cancels and the dragged element reverts to its exact drag-start state — for every element kind (room move/resize, opening, wall item, floor item, furniture rotation), not just colliding solid floor items. This doubles as the touch replacement for Escape-cancel, though Escape itself keeps its existing collision-only revert (it only ever needed to undo a stray collision, since Escape has no "second finger" equivalent forcing a full revert).
